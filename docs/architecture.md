...
All Module and View MDDs form one implementation dependency graph containing compile, Luban authoring, registration, non-Prefab resource, and runtime-contract edges. Prefab creation, modification, layout, serialized binding, and Inspector work remain a separate manual UI-art stage: View MDD prose records the expected resource name and path, hierarchy/layout, binding requirements, safe area, and manual acceptance, but Prefab never appears as a structured artifact, product symbol, dependency edge, implementation-order node, or whitelisted file. The graph is topologically ordered before numbering. Every MDD must compile and complete its automated acceptance using only the stable host, its own whitelist, and earlier MDD outputs. A cycle indicates incorrect ownership, action boundaries, navigation, or MDD boundaries and is fixed in design; events, interfaces, string routing, placeholders, or Common/Core containers are not used to conceal it.
...
Validation is proportional to the artifact: Unity compilation and Console review, Luban full generation, registration checks, non-Prefab resource checks, representative WindowData/GM snapshots, repeatable editor/manual flow, and target-platform checks when applicable. Automated implementation validation does not inspect or accept Prefab/Inspector state; it verifies that the View MDD contains a complete manual Prefab handoff, while physical Prefab and Inspector acceptance occurs separately after that manual stage exists.
