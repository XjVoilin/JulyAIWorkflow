...
在已经由 July 模板创建好的 Unity 项目中开启 Codex 任务，只使用这一条统一口令：
...
如果实施时发现 GDD、模块设计或 MDD 无法正确落地，插件会在同一次“执行”中进入设计一致性修复：仅属于 MDD 技术契约的问题直接修正并继续；涉及模块职责时说明方案并询问一次；涉及产品语义时只问一个产品问题。用户确认后，插件按 GDD → 模块设计 → 完整 MDD 集合的顺序同步并验证，然后继续原 MDD，不要求用户手动编辑文档或重新执行命令。

## 执行规则

- 对策划案、GDD 或模块设计说“执行”时，如果紧邻的下游目标不存在就直接生成；目标已存在则先询问是否更新。
- 对 MDD 说“执行”时直接进入实施前检查；已有代码由 MDD 决定创建、修改或复用，不额外询问是否更新代码。
- 实施中的设计修复属于当前“执行”的内部闭环，不是新的命令，也不授权实现其他 MDD。
- 策划案、GDD 或模块设计输入的目标更新确认只授权紧邻阶段；MDD 实施中的设计修复确认只授权恢复当前实施所必需的上游修复和下游一致性同步。
- “生成”“更新”“重新生成”“实施”不是独立启动命令；缺少“执行”或使用其他动作启动时，插件会提示统一口令并停止。插件询问是否更新后，回答“是”或“更新”只是本次执行的确认。

## 文件定位

- 必须通过 `@` 明确指定且只指定一份 Markdown 文件；目录可以叫 `DesignDoc`，但不要求固定名称。
- 找不到文件或存在多个无法判断的候选时，插件会说明未执行，并要求用户明确指定。
- 新生成的 GDD、模块设计和 MDD 都写入稳定文档类型标识；已有规范产物可按旧结构兼容识别。
- 下游目标不存在时创建；已存在时只有在用户对当前询问作出肯定确认后才更新。
...
Skill 保持最小结构。规则集中在入口和七份按需加载的参考文档中，不依赖脚本、流程状态文件或额外机器契约。
