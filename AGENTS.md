...
- During complete-design generation, do not write formal project design files incrementally. Build the machine-readable contract and all artifacts in the OS temporary staging directory created by `scripts/design_artifacts.py`; every staging validation must bind the same workspace and unchanged planning input, then use the script's transactional publisher.
...
- Give every atomic player action one global action ID, owner, canonical signature, failure contract, and navigation owner. Module and View documents must reference that contract without redefining it.
- Give every cross-MDD product symbol one providing MDD. Build one acyclic implementation graph containing all Module and View MDDs plus compile, Luban authoring, registration, and runtime-contract edges. Prefabs are manual UI handoffs documented in View prose, not product symbols or graph edges.
- Topologically order the entire graph before assigning M/V numbers. Every MDD must compile and complete its own acceptance using only the stable host, its own whitelist, and earlier MDD outputs. Resolve cycles by correcting ownership and responsibility, not by adding events, interfaces, string routing, Common/Core containers, placeholders, or coordination layers.
- Do not reuse an installed persistence capability for product Stores. Do not design persistence registration, save/load calls or timing, save failure, cross-launch recovery, repositories, server entities, migrations, or placeholders. Persistence is intentionally deferred to the product owner.
- Do not generate target-project unit tests, PlayMode tests, test asmdefs, mocks, fakes, or fixtures. Validate with compilation, Unity Console, Luban full generation, non-Prefab resource checks, repeatable editor/manual flows, and platform checks when relevant. Prefab/Inspector production and visual acceptance belong to a separate manual UI stage.
- Keep regression tests for the plugin's own validator and publisher under the plugin's `scripts/tests`; this does not authorize tests in a target Unity product.
- Each MDD contains an exact product-file whitelist. Implementation may only create or modify those files plus generated Luban outputs and already-declared registration/configuration files. `.prefab` files are forbidden from structured contracts and whitelists; a View instead records the expected Prefab name, path, layout, binding, and manual acceptance in prose.
