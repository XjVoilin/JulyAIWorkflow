...
description: 仅在用户显式调用后，根据用户 @ 指定的策划案、GDD、模块设计或单份 MDD 执行唯一下一步；也支持用明确动作覆盖自动路由。适用于使用 JulyFramework 与 Luban 的 Unity 项目；不生成图片、Prefab 或测试。
---
...
`索引.md` 不是可执行 MDD。任意普通 Markdown 不自动视为策划案。类型标识与实际结构冲突、同时符合多个类型、文件不存在或无法唯一识别时，停止并要求用户修正文档或明确动作，不猜测。

用户只说“执行”时必须 `@` 且只 `@` 一份文档。根据输入类型决定动作，不根据下游目标是否存在反推阶段。

## 默认执行与显式覆盖

对策划案、GDD 或模块设计使用“执行”时：

- 下游目标不存在：直接按“生成”执行；
- 下游目标已经存在：先询问用户是否更新，未获得确认前不修改；
- 一次确认只授权更新当前紧邻的下一阶段，不自动更新更下游产物。

对一份 MDD 使用“执行”时，直接进入该 MDD 的实施前检查。已有代码不是需要再次询问的“下游目标”；按照 MDD 判断创建、修改或复用。

用户给出明确动作时以该动作覆盖“执行”的默认选择，但输入文档类型必须与动作匹配：

- “生成”“更新”或“重新生成 GDD”要求输入策划案；
- “生成”或“更新模块设计”要求输入 GDD；
- “生成”或“更新全部 MDD”要求输入模块设计；
- “实施”要求输入且只输入一份 MDD。

明确动作与输入类型冲突时停止，不通过改解释动作来继续。没有 `@` 但动作明确时，只使用当前项目中名称和结构都明确且唯一的对应输入；找不到或存在多个候选时询问，不自行猜测。
