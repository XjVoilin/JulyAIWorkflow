...
- 能在当前动作授权范围内修正的问题应立即修正并重新验证；遇到需要用户决定的关键歧义或超出当前动作的冲突时暂停，不报告完成。实施 MDD 时继续进入设计一致性修复，其他阶段停止并询问。
- 实施 MDD 时发现设计链问题，不把手动修改上游文档和重新执行命令交给用户；按照实施路由进入一致性修复闭环。
...
实施前检查或实施过程中发现 GDD、模块设计、MDD 之间存在缺失、冲突或无法正确落地的设计时，再读取 [references/design-repair.md](references/design-repair.md)。修复闭环是当前“执行”的内部步骤，不是新命令，也不是自动推进无关阶段。
