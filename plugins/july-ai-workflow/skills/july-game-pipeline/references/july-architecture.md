...
- 不直接编辑生成产物；
- 运行时变化仍由 Store 拥有。
- 只转发 `IConfigSystem`、`GetTable` 或表查询的类型不创建为项目 System，直接使用框架配置能力。
- 只有一次性配置校验时使用普通校验类型；同时承担集中校验和多个调用方需要的领域查询时，才创建具体的 `<领域名>ConfigSystem`，名称必须反映真实业务范围。
- 必需配置在加载边界统一校验。校验通过后内部查询使用 `Get...` 并在违约时快速失败；只有配置缺失是明确允许的业务状态时才使用 `TryGet...`。
