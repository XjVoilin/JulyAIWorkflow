...
每份 `Design/Docs/MDD/Views/V<N>_<视觉功能>.md` 描述一个玩家可见屏幕或视觉功能。目录不按 UI 与 2D 技术类型拆分；在 MDD 内声明实际形态。View 必须位于索引全产物拓扑序中，所用模块类型、生成类型、事件和非 Prefab 资源必须已经由更早 MDD 提供或由当前白名单创建。Prefab 是独立人工 UI 交付，不进入结构化合同或实施闭包。
...
有设计图、现有 Prefab 或场景时必须实际查看并记录信息层级、交互和制作约束。查看现有 Prefab 不授权修改它，也不把它加入当前白名单。
...
Prefab 行只定义人工交付合同：资源名、预期路径、需要挂载的 Window、布局层级、序列化引用、安全区和人工验收方式。必须明确写出“后续人工制作，不进入本 MDD 白名单”。Prefab 不得成为 `provides`、`consumes`、`dependsOn`、`implementationOrder` 或任何文件白名单项。非 Prefab 图片、字体、本地化和场景资产仍按实际责任进入白名单。
...
```text
<逐个列出 WindowData、Window/GameView、Events、非 Prefab 资源等准确文件>
```

### 修改

```text
<逐个列出 Window 配置、场景、本地化等准确文件及目的；不得列出 .prefab>
```
...
逐项覆盖：Window 基类与配置、WindowData 使用的 Store/System、字段中的生成类型、业务事件、按钮规范调用、本地化与非 Prefab 资源。Prefab 人工交付说明不作为闭包输入。

闭包结论必须明确：

- 当前白名单一起创建后可以 Unity 编译；
- 不引用后续 Module/View 的类型或资源；
- 当前脚本、Data、配置、非 Prefab 资源与 GM 代表数据可以在不打开 Window 的前提下验收；
- Prefab 未交付时不打开 Window、不创建空界面或伪成功；Prefab/Inspector 与最终可视化验收留给独立人工 UI 阶段；
...
- GM/编辑器以可写 Data 检查代表数据；只有已存在人工 Prefab 时才可另行打开 Window，该结果不属于本 MDD 自动验收；
- 场景引用、非 Prefab 资源与适用的目标分辨率；
- 正文中的 Prefab 名称、预期路径、布局和人工接线说明完整，且实际文件差异不含 `.prefab`；
- GameView 表现与交互路径；
- 文件白名单一致；
- 没有业务规则、持久化占位或测试代码。
...
- Prefab 创建或修改、层级布局、节点绑定、图片挂载、Prefab Inspector 接线与窗口可视化验收。
